///////////////////////////////////////////////////////////////////////
/// File    : LasPointReader.cs
/// Desc    : Las point reader.
/// Author  : Li G.Q.
/// Date    : 2021/9/13/
///////////////////////////////////////////////////////////////////////

using LasLibNet.Abstract;
using LasLibNet.Implement;
...
namespace LasLibNet
{
	...
	public class LasPointReader
	{
		...
		public bool setup(uint num_items, LASitem[] items, LASzip laszip = null)
		{
			...
			readers_raw = new LasItemReader[num_readers];
			for (int i = 0; i < num_readers; i++)
			{
				switch (items[i].type)
				{
					case LASitem.Type.POINT10: readers_raw[i] = new LasRawItemReader_POINT10(); break;
					case LASitem.Type.GPSTIME11: readers_raw[i] = new LasRawItemReader_GPSTIME11(); break;
					case LASitem.Type.RGB12: readers_raw[i] = new LasRawItemReader_RGB12(); break;
					case LASitem.Type.WAVEPACKET13: readers_raw[i] = new LasRawItemReader_WAVEPACKET13(); break;
					case LASitem.Type.BYTE: readers_raw[i] = new LasRawItemReader_BYTE(items[i].size); break;
					case LASitem.Type.POINT14: readers_raw[i] = new LasRawItemReader_POINT14(); break;
					case LASitem.Type.RGBNIR14: readers_raw[i] = new LasRawItemReader_RGBNIR14(); break;
			...
			if (dec != null)
			{
				readers_compressed = new LasItemReader[num_readers];
				...
				for (int i = 0; i < num_readers; i++)
				{
					switch (items[i].type)
					{
						case LASitem.Type.POINT10:
							if (items[i].version == 1) readers_compressed[i] = new LasCompressedItemReader_POINT10_v1(dec);
							else if (items[i].version == 2) readers_compressed[i] = new LasCompressedItemReader_POINT10_v2(dec);
							else return false;
							break;
						case LASitem.Type.GPSTIME11:
							if (items[i].version == 1) readers_compressed[i] = new LasCompressedItemReader_GPSTIME11_v1(dec);
							else if (items[i].version == 2) readers_compressed[i] = new LasCompressedItemReader_GPSTIME11_v2(dec);
							else return false;
							break;
						case LASitem.Type.RGB12:
							if (items[i].version == 1) readers_compressed[i] = new LasCompressedItemReader_RGB12_v1(dec);
							else if (items[i].version == 2) readers_compressed[i] = new LasCompressedItemReader_RGB12_v2(dec);
							else return false;
							break;
						case LASitem.Type.WAVEPACKET13:
							if (items[i].version == 1) readers_compressed[i] = new LasCompressedItemReader_WAVEPACKET13_v1(dec);
							else return false;
							break;
						case LASitem.Type.BYTE:
							seek_point.extra_bytes = new byte[items[i].size];
							seek_point.num_extra_bytes = items[i].size;
							if (items[i].version == 1) readers_compressed[i] = new LasCompressedItemReader_BYTE_v1(dec, items[i].size);
							else if (items[i].version == 2) readers_compressed[i] = new LasCompressedItemReader_BYTE_v2(dec, items[i].size);
							else return false;
							break;
						default: return false;
					}
		...
		public bool init(Stream instream)
		{
			...
			for (int i = 0; i < num_readers; i++)
			{
				((LasRawItemReader)(readers_raw[i])).Init(instream);
			}
		...
		public bool read(LasPoint point)
		{
			try
			{
				if (dec != null)
				{
					...
					else
					{
						for (int i = 0; i < num_readers; i++)
						{
							...
							((LasCompressedItemReader)(readers_compressed[i])).Init(point);
						}
		...
		Stream instream;
		uint num_readers;
		LasItemReader[] readers;
		LasItemReader[] readers_raw;
		LasItemReader[] readers_compressed;
		...
		long point_start;
		uint point_size;
		LasPoint seek_point = new LasPoint();
		string last_error;
	}
}
