...
namespace LasLibNet.Reader
{
	class LazPointReader
	{
		...
		LazItemReader[] readers;
		LazItemReader[] readers_compressed;
		...
		/// </summary>
		/// <param name="lasReader">Raw point reader</param>
		public LazPointReader(ILasPointReader lasPointReader)
		{
			point_size=0;
			instream=null;
			num_readers=0;
			...
			this.lasPointReader = lasPointReader;
		}

		// should only be called *once*
		public bool setup( LazFile laszip)
		{ 
			uint num_items = laszip.num_items;
			LazItem[] items = laszip.items;
			...
			if(laszip!=null&&laszip.compressor!=0)
			{
				switch(laszip.coder)
				{
					case LazFile.CODER_ARITHMETIC: dec=new ArithmeticDecoder(); break;
			...
			if(dec!=null)
			{
				readers_compressed=new LazItemReader[num_readers];
					...
					switch(items[i].type)
					{
						case LazItem.Type.POINT10:
							if(items[i].version==1) 
								readers_compressed[i]=new LazItemReader_POINT10_v1(dec);
							else if(items[i].version==2) 
								readers_compressed[i]=new LazItemReader_POINT10_v2(dec);
							else return false;
							break;
						case LazItem.Type.GPSTIME11:
							if(items[i].version==1) 
								readers_compressed[i]=new LazItemReader_GPSTIME11_v1(dec);
							else if(items[i].version==2) 
								readers_compressed[i]=new LazItemReader_GPSTIME11_v2(dec);
							else return false;
							break;
						case LazItem.Type.RGB12:
							if(items[i].version==1) 
								readers_compressed[i]=new LazItemReader_RGB12_v1(dec);
							else if(items[i].version==2) 
								readers_compressed[i]=new LazItemReader_RGB12_v2(dec);
							else return false;
							break;
						case LazItem.Type.WAVEPACKET13:
							if(items[i].version==1) 
								readers_compressed[i]=new LazItemReader_WAVEPACKET13_v1(dec);
							else return false;
							break;
						case LazItem.Type.BYTE:
							...
							if(items[i].version==1) 
								readers_compressed[i]=new LazItemReader_BYTE_v1(dec, items[i].size);
							else if(items[i].version==2) 
								readers_compressed[i]=new LazItemReader_BYTE_v2(dec, items[i].size);
							else return false;
							break;
				...
				if(laszip.compressor==LazFile.COMPRESSOR_POINTWISE_CHUNKED)
				{
		...
		{			
			try
			{
				if (dec != null)
				{
					...
					{
						// Read the raw point.
						point = lasPointReader.Read();
						//Initialize the compressed-point reader.
						for (int i = 0; i < num_readers; i++)
						{
							readers_compressed[i].Init(point);
						}
			...
			{
				// end-of-file
				if(dec!=null) error="end-of-file during chunk "+current_chunk;
			...
			{
				// decompression error
				error=string.Format("chunk {0} of {1} is corrupt. Error: {2}"
					, current_chunk, tabled_chunks,ex.Message);
