...
namespace LasLibNet
{
	class LasPointWriter
	{
		...
		public bool setup(uint num_items, LasItem[] items, LasFile lasfile=null)
		{
			// is laszip exists then we must use its items
			if(lasfile!=null)
			{
				if(num_items!=lasfile.num_items) return false;
				if(items!=lasfile.items) return false;
			}

			// create entropy encoder (if requested)
			enc=null;
			if(lasfile!=null&&lasfile.compressor!=0)
			{
				switch(lasfile.coder)
				{
					case LasFile.CODER_ARITHMETIC: enc=new ArithmeticEncoder(); break;
			...
			writers_raw=new LasItemWriter[num_writers];

			for(uint i=0; i<num_writers; i++)
			{
				switch(items[i].type)
				{
					case LasItem.Type.POINT10: writers_raw[i]=new LasRawItemWriter_POINT10(); break;
					case LasItem.Type.GPSTIME11: writers_raw[i]=new LasRawItemWriter_GPSTIME11(); break;
					case LasItem.Type.RGB12: writers_raw[i]=new LasRawItemWriter_RGB12(); break;
					case LasItem.Type.WAVEPACKET13: writers_raw[i]=new LasRawItemWriter_WAVEPACKET13(); break;
					case LasItem.Type.BYTE: writers_raw[i]=new LasRawItemWriter_BYTE(items[i].size); break;
					case LasItem.Type.POINT14: writers_raw[i]=new LasRawItemWriter_POINT14(); break;
					case LasItem.Type.RGBNIR14: writers_raw[i]=new LasRawItemWriter_RGBNIR14(); break;
			...
			if(enc!=null)
			{
				writers_compressed=new LasCompressedItemWriter[num_writers];
				...
				for(uint i=0; i<num_writers; i++)
				{
					switch(items[i].type)
					{
						case LasItem.Type.POINT10:
							...
							else if(items[i].version==2) writers_compressed[i]=new LasCompressedItemWriter_POINT10_v2(enc);
							else return false;
							break;
						case LasItem.Type.GPSTIME11:
							...
							else if(items[i].version==2) writers_compressed[i]=new LasCompressedItemWriter_GPSTIME11_v2(enc);
							else return false;
							break;
						case LasItem.Type.RGB12:
							...
							else if(items[i].version==2) writers_compressed[i]=new LasCompressedItemWriter_RGB12_v2(enc);
							else return false;
							break;
						case LasItem.Type.WAVEPACKET13:
							if(items[i].version==1) writers_compressed[i]=new LasCompressedItemWriter_WAVEPACKET13_v1(enc);
							else return false;
							break;
						case LasItem.Type.BYTE:
							...
							else if(items[i].version==2) writers_compressed[i]=new LasCompressedItemWriter_BYTE_v2(enc, items[i].size);
							else return false;
							break;
						default: return false;
					}
				...
				if(lasfile.compressor==LasFile.COMPRESSOR_POINTWISE_CHUNKED)
				{
					if(lasfile.chunk_size!=0) chunk_size=lasfile.chunk_size;
					chunk_count=0;
					init_chunking=true;
				}
			}
		...
		public bool init(Stream outstream)
		{
			...
			for(uint i=0; i<num_writers; i++)
			{
				((LasRawItemWriter)(writers_raw[i])).init(outstream);
			}
		...
		public bool write(LasPoint point)
		{
			...
			else
			{
				for(uint i=0; i<num_writers; i++)
				{
					...
					((LasCompressedItemWriter)(writers_compressed[i])).init(point);
				}
		...
		Stream outstream;
		uint num_writers;
		LasItemWriter[] writers;
		LasItemWriter[] writers_raw;
		LasItemWriter[] writers_compressed;
